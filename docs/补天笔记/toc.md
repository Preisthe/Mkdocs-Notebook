...
- 正则语言 𝐿 中所有字符串的反转构成的集合 𝐿^𝑅 也是正则语言（可通过构造 NFA 证明）
- 单个正则语言是可数的（语言 𝐿 ⊆ Σ* ，若 Σ 有限，那么 Σ* 就是可数的）
- 所有正则语言构成的集合也是可数的
...
- PDA 的转移关系Δ：(K×(Σ∪{e})×Γ*) × (K×Γ*) 的有限子集
    - K 为状态
    - Σ 为输入字母表
    - Γ 为栈字母表
    - 左边是读取的字符（或不读取）和要弹出的字符串，右边是要压入的字符串
- PDA 的配置定义为 K×Σ∗×Γ∗ 的一个成员，表示栈中所有的字符
...
- 复合图灵机的例子：
  该机器从 M 1 的初始状态开始，先由 M 1 操作，直到其停机 若停机时，当前扫描到的符号为 a ，那么就启动 M 2 并以 M 2 完成后续操作；若扫描到的符号为 b ，那么就启动 M 3 并以 M 3 完成后续操作
- M 在输入 w 上的初始配置(initial configuration) 为 (s, ▹, ⊔w)，读写头在空格上
- 递归可枚举：若停则接受，也称半判定
- 递归语言的补也是递归的
...
- 无限制文法的规则： (V* (V−Σ) V*) × V* 的有限子集
- 当且仅当语言是递归可枚举时，它由文法生成
- 由 SwS 生成的 f(w) 是文法可计算的
